...
| variant | interface + concrete types | `C.FfiVariantName` (tag + named ptrs) | C -> Go |
| enum | `type Foo uint32` + constants | `C.uint32_t` tag | C -> Go |
| `result<T, E>` | `(T, error)` return | success ptr or `last_error` | C -> Go |

## C-ABI Variant Representation

The Rust-generated C-ABI represents variants as a struct with a `tag` field and
**named pointer fields** for each case (not a C `union`):
...
Each payload wrapper is a struct containing a single `value` field:

```c
typedef struct { FfiNativeRequest value; } FfiTransactionRequestNativePayload;
```

On the Go side, the active pointer is non-nil and the others are nil. Access
the payload by dereferencing the active pointer and reading its `.value` field.

## C-ABI Error Reporting

The C-ABI does **not** expose a single `last_error()` function returning
`FfiByteBuffer`. Instead, errors are reported via three functions:
...
The Go `readLastError()` helper must use the length+copy pattern (matching
what the Swift generator already produces).

## C-ABI Struct Field Names

The `FfiByteSlice` and `FfiByteBuffer` structs use `ptr`/`len` (not
`data`/`len`):

```c
typedef struct { const uint8_t *ptr; size_t len; } FfiByteSlice;
typedef struct { uint8_t *ptr; size_t len; } FfiByteBuffer;
```

All generated Go code must reference `buf.ptr` and `buf.len`, not `buf.data`.

## C-ABI Function Naming

Exported functions follow the pattern `{c_prefix}_{interface}_{function}`:
...
Note that `result<T, E>` functions return a **pointer** (null on error), while
non-result functions return the value **directly** (e.g., `FfiByteBuffer`).
...
The following Go name helpers **already exist**:
- `to_go_type(name) -> String` — PascalCase (exported types)
- `to_go_func(name) -> String` — PascalCase (exported functions)
- `to_go_field(name) -> String` — PascalCase (exported struct fields)
...
(e.g., marker interface names like `transactionRequestVariant`, conversion
...
`fmt.Errorf`) or if any variant types exist (for the `panic` in the default
switch case).
...
- `readLastError() string` — uses the length+copy error API:
  1. Call `{c_prefix}_last_error_length()` to get the length
  2. If length <= 0, return `"unknown error"`
  3. Allocate a `[]byte` buffer of that length
  4. Call `{c_prefix}_error_message_utf8(buf, len)` to copy the message
  5. Return the string
...
Swift generator — each generator has its own copy of this method).
...
For variants, switch on the tag field and access the **named pointer fields**
(not a union). Each payload pointer wraps a struct with a `.value` field:
...
One function per exported WIT function. There are two patterns depending on
whether the WIT function returns `result<T, E>` or a plain value.

**Pattern 1: `result<T, E>` return** — The C function returns a pointer (null
on error). The Go function returns `(T, error)`. The pointer must be freed with
the type-specific free function (not `C.free`), since the struct may contain
nested allocations:
...
**Pattern 2: Plain return** — The C function returns the value directly (not a
pointer). No null check needed. The appropriate helper handles freeing:

```go
// U256ToString converts a u256 to a string for display.
func U256ToString(input []byte) string {
    inputSlice := C.FfiByteSlice{
...
- **Result types:** Check for null pointer, call `readLastError()` on failure,
  free with the **type-specific** free function (e.g.,
  `{c_prefix}_free_transaction_request`)
- **Non-result returns:** Directly convert via the appropriate helper or
  conversion function — no null check
...
### Step 6: xtask integration

**File:** `crates/xtask/src/lib.rs`

The `xtask` crate provides a `generate(workspace_root)` function that produces
all FFI artifacts in one shot. It currently generates Rust, C headers, Kotlin,
and Swift. Go must be added:

- Add `GenerateGo { source: witffi_go::generate::Error }` variant to the
  `Error` enum
- Add a Go output path constant:
  ```rust
  const GO_OUTPUT: &str = "examples/eip681-go/bindings.go";
  ```
- Add a Go generation block in `generate()` after the Swift section:
  ```rust
...
- Add `witffi-go.workspace = true` to `[dependencies]`
...
All existing 30 tests must continue to pass. The new Go generator should add at
least 3 more tests.
...
Go 1.20+ since it's been stable for over 3 years.
