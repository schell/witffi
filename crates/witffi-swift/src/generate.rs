...
impl<'a> SwiftGenerator<'a> {
    ...
    /// # Errors
    ///
    /// Returns an error if code generation fails (e.g. unsupported WIT
    /// type patterns).
    pub fn generate(&self) -> anyhow::Result<String> {
        ...
        writeln!(out)?;
        self.generate_error_type(&mut out)?;
        writeln!(out)?;
        self.generate_helpers(&mut out)?;
        writeln!(out)?;
        self.generate_types(&mut out)?;
        writeln!(out)?;
        self.generate_api(&mut out)?;
    ...
    fn generate_header(&self, out: &mut String) -> anyhow::Result<()> {
    ...
    fn generate_error_type(&self, out: &mut String) -> anyhow::Result<()> {
    ...
    fn generate_helpers(&self, out: &mut String) -> anyhow::Result<()> {
        let prefix = self.c_func_prefix();
    ...
    fn generate_types(&self, out: &mut String) -> anyhow::Result<()> {
        ...
        for type_id in &reachable {
            self.generate_type_def(out, *type_id)
                .with_context(|| format!("generating Swift type for {:?}", type_id))?;
        }
    ...
    fn generate_conversion_functions(&self, out: &mut String) -> anyhow::Result<()> {
    ...
    fn generate_api(&self, out: &mut String) -> anyhow::Result<()> {
    ...
    fn generate_api_function(&self, out: &mut String, ef: &ExportedFunction) -> anyhow::Result<()> {
