...
impl<'a> RustGenerator<'a> {
    ...
    pub fn generate(&self) -> anyhow::Result<String> {
        ...
        writeln!(out)?;
        writeln!(out, "use std::ffi::{{CStr, CString}};")?;
        writeln!(out, "use std::os::raw::c_char;")?;
        writeln!(out, "use std::ptr;")?;
        writeln!(out)?;

        self.generate_types(&mut out)?;
        writeln!(out)?;
        self.generate_trait(&mut out)?;
        writeln!(out)?;
        self.generate_ffi_functions(&mut out)?;
        writeln!(out)?;
        self.generate_free_functions(&mut out)?;

        Ok(out)
    }

    /// Generate a C header string.
    pub fn generate_c_header(&self) -> anyhow::Result<String> {
        let mut out = String::new();
        ...
        self.generate_c_types(&mut out)?;
        writeln!(out)?;
        self.generate_c_function_decls(&mut out)?;
        writeln!(out)?;
        self.generate_c_free_decls(&mut out)?;
    ...
    fn generate_types(&self, out: &mut String) -> anyhow::Result<()> {
        writeln!(out, "// ---- FFI-safe types ----")?;
        writeln!(out)?;

        writeln!(out, "/// An FFI-safe byte buffer (caller or callee owned).")?;
        writeln!(out, "#[repr(C)]")?;
        writeln!(out, "#[derive(Debug)]")?;
        writeln!(out, "pub struct FfiByteSlice {{")?;
        writeln!(out, "    pub ptr: *const u8,")?;
        writeln!(out, "    pub len: usize,")?;
        writeln!(out, "}}")?;
        writeln!(out)?;

        writeln!(
            out,
            "/// An FFI-safe owned byte buffer (must be freed by caller)."
        )?;
        writeln!(out, "#[repr(C)]")?;
        writeln!(out, "#[derive(Debug)]")?;
        writeln!(out, "pub struct FfiByteBuffer {{")?;
        writeln!(out, "    pub ptr: *mut u8,")?;
        writeln!(out, "    pub len: usize,")?;
        writeln!(out, "}}")?;
        writeln!(out)?;
        ...
        for type_id in &reachable {
            self.generate_type_def(out, *type_id)
                .with_context(|| format!("generating type {:?}", type_id))?;
        }

        Ok(())
    }

    fn generate_type_def(&self, out: &mut String, type_id: TypeId) -> anyhow::Result<()> {
        let typedef = &self.resolve.types[type_id];
            ...
            Type::Id(id) => {
                ...
                match &typedef.kind {
                    TypeDefKind::List(inner) => match inner {
                        Type::U8 => "FfiByteBuffer".to_string(),
                        _ => format!("FfiByteBuffer /* list<{}> */", self.type_to_c_rust(inner)),
                    },
    ...
    fn generate_trait(&self, out: &mut String) -> anyhow::Result<()> {
        let world = &self.resolve.worlds[self.world_id];
            ...
            let params: Vec<String> = ef
                ...
                .map(|p| {
                    format!(
                        ...
                        self.type_to_trait_rust(&p.ty)
                    )
                })
                .collect();

            let ret = self.function_return_to_trait_rust(&ef.function.result);
    ...
    /// Map a WIT type to its idiomatic Rust representation (for the trait).
    fn type_to_trait_rust(&self, ty: &Type) -> String {
        match ty {
            Type::Bool => "bool".to_string(),
            Type::U8 => "u8".to_string(),
            Type::U16 => "u16".to_string(),
            ...
            Type::Id(id) => {
                ...
                match &typedef.kind {
                    ...
                    TypeDefKind::List(inner) => {
                        format!("Vec<{}>", self.type_to_trait_rust(inner))
                    }
                    TypeDefKind::Option(inner) => {
                        format!("Option<{}>", self.type_to_trait_rust(inner))
                    }
                    TypeDefKind::Result(r) => {
                        let ok_ty =
                            r.ok.as_ref()
                                .map(|t| self.type_to_trait_rust(t))
                                .unwrap_or_else(|| "()".to_string());
                        let err_ty = r
                            .err
                            .as_ref()
                    ...
                    TypeDefKind::Type(aliased) => self.type_to_trait_rust(aliased),
                    _ => {
                        let name = typedef.name.as_deref().unwrap_or("Anonymous");
    ...
    fn function_return_to_trait_rust(&self, result: &Option<Type>) -> String {
        match result {
            Some(ty) => self.type_to_trait_rust(ty),
            None => "()".to_string(),
        }
    }
    ...
    fn generate_ffi_functions(&self, out: &mut String) -> anyhow::Result<()> {
        writeln!(out, "// ---- FFI functions ----")?;
        writeln!(out)?;
        ...
        writeln!(
            out,
            "/// `buf` must point to `len` bytes of writable memory."
        )?;
        writeln!(out, "#[unsafe(no_mangle)]")?;
        writeln!(
            out,
            "pub unsafe extern \"C\" fn {prefix}_error_message_utf8(buf: *mut c_char, len: i32) -> i32 {{"
        )?;
        writeln!(out, "    LAST_ERROR.with(|e| {{")?;
        ...
        writeln!(
            out,
            "                unsafe {{ ptr::copy_nonoverlapping(bytes.as_ptr(), buf as *mut u8, copy_len) }};"
        )?;
        ...
        writeln!(out, "#[unsafe(no_mangle)]")?;
        writeln!(out, "pub extern \"C\" fn {prefix}_clear_last_error() {{")?;
        writeln!(out, "    LAST_ERROR.with(|e| *e.borrow_mut() = None);")?;
        writeln!(out, "}}")?;
        writeln!(out)?;

        let funcs = exported_functions(self.resolve, self.world_id);
        for ef in &funcs {
            self.generate_ffi_function(out, ef)?;
        }

        Ok(())
    }

    fn generate_ffi_function(&self, out: &mut String, ef: &ExportedFunction) -> anyhow::Result<()> {
        ...
        // Convert parameters from C types to Rust types
        writeln!(
            out,
        ...
        writeln!(out, "        T::{trait_method}({})", rust_args.join(", "))?;
        writeln!(out, "    }}));")?;
        writeln!(out)?;
    ...
    fn generate_param_conversion(
        ...
        match ty {
            Type::String => {
                writeln!(
                    out,
                    "        let {c_name}_rust = unsafe {{ std::str::from_utf8_unchecked(std::slice::from_raw_parts({c_name}.ptr, {c_name}.len)) }};"
                )?;
            }
            Type::Bool
            | Type::U8
            ...
            | Type::F32
            | Type::F64 => {
                writeln!(out, "        let {c_name}_rust = {c_name};")?;
            }
            Type::Id(id) => {
                let typedef = &self.resolve.types[*id];
                match &typedef.kind {
                    TypeDefKind::List(Type::U8) => {
                        writeln!(
                            out,
                            "        let {c_name}_rust = unsafe {{ std::slice::from_raw_parts({c_name}.ptr, {c_name}.len) }};"
                        )?;
                    }
                    _ => {
                        writeln!(out, "        let {c_name}_rust = {c_name};")?;
    ...
    fn generate_free_functions(&self, out: &mut String) -> anyhow::Result<()> {
        ...
        writeln!(
            out,
            "pub unsafe extern \"C\" fn {prefix}_free_byte_buffer(buf: FfiByteBuffer) {{"
        )?;
        writeln!(out, "    if !buf.ptr.is_null() && buf.len > 0 {{")?;
        writeln!(
            out,
        ...
        for type_id in &reachable {
            ...
            match &typedef.kind {
                TypeDefKind::Record(_) | TypeDefKind::Variant(_) => {
                    ...
                    writeln!(out, "    if !ptr.is_null() {{")?;
                    writeln!(out, "        drop(unsafe {{ Box::from_raw(ptr) }});")?;
                    writeln!(out, "    }}")?;
                    writeln!(out, "}}")?;
                    writeln!(out)?;
                }
    ...
    fn generate_c_types(&self, out: &mut String) -> anyhow::Result<()> {
        writeln!(out, "/* ---- Types ---- */")?;
        writeln!(out)?;

        writeln!(out, "typedef struct {{")?;
        writeln!(out, "    const uint8_t *ptr;")?;
        writeln!(out, "    size_t len;")?;
        writeln!(out, "}} FfiByteSlice;")?;
        writeln!(out)?;
        writeln!(out, "typedef struct {{")?;
        writeln!(out, "    uint8_t *ptr;")?;
        writeln!(out, "    size_t len;")?;
        writeln!(out, "}} FfiByteBuffer;")?;
        writeln!(out)?;
    ...
    fn generate_c_type_def(&self, out: &mut String, type_id: TypeId) -> anyhow::Result<()> {
        let typedef = &self.resolve.types[type_id];
        let wit_name = typedef.name.as_deref().unwrap_or("anonymous");
    ...
    fn generate_c_function_decls(&self, out: &mut String) -> anyhow::Result<()> {
    ...
    fn generate_c_free_decls(&self, out: &mut String) -> anyhow::Result<()> {
        writeln!(out, "/* ---- Free functions ---- */")?;
        writeln!(out)?;
...
mod tests {
    ...
    fn test_generate_rust_from_eip681() {
        ...
        assert!(
            ...
            "missing FfiByteBuffer"
        );
        assert!(code.contains("pub trait Eip681"), "missing trait Eip681");
        assert!(
            code.contains("pub unsafe extern \"C\" fn zcash_eip681_parser_parse"),
            "missing parser_parse fn"
        );
        assert!(
    ...
    fn test_generate_c_header_from_eip681() {
        ...
        assert!(
            header.contains("zcash_eip681_parser_parse"),
            "missing parser_parse"
        );
    }
}
